...
describe('Render tests', () => {
  ...
  test('Expects 3 disabled timeslots based on different currentDate prop sent.', () => {
    const component = mount(
      <Calendar
        currentDate = { moment([2017, 3, 29]).format() }
        initialDate = { moment([2017, 3, 30]).format() }
        timeslots = { DEFAULT_TIMESLOTS }
        disabledTimeslots = { [
          {
    ...
    expect(disabledTimeslots).toHaveLength(168);
  });

  test('Expects 3 disabled timeslots based on different currentDate prop sent 2.', () => {
    const component = mount(
      <Calendar
        currentDate = { moment([2017, 3, 30]).format() }
        initialDate = { moment([2017, 3, 29]).format() }
        timeslots = { DEFAULT_TIMESLOTS }
        disabledTimeslots = { [
          {
            startDate: 'April 30th 2017, 12:00:00 AM',
